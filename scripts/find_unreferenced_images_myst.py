...
def save_results(results, include_ai=True):
    ...
    if results['unreferenced_images']:
        with open('unreferenced_images.txt', 'w', encoding='utf-8') as f:
            ...
            for image in sorted(results['unreferenced_images']):
                f.write(f"{image}\n")
        print(f"  Saved {len(results['unreferenced_images'])} unreferenced images to 'unreferenced_images.txt'")

    # Save unreferenced .ai files if requested
    if include_ai and results['unreferenced_ai_files']:
        with open('unreferenced_ai_files.txt', 'w', encoding='utf-8') as f:
            ...
            for ai_file in sorted(results['unreferenced_ai_files']):
                ...
                f.write("\n")
        print(f"  Saved {len(results['unreferenced_ai_files'])} unreferenced .ai files to 'unreferenced_ai_files.txt'")

    # Save referenced images for verification
    with open('referenced_images.txt', 'w', encoding='utf-8') as f:
        ...
        for image in sorted(results['referenced_images']):
            f.write(f"{image}\n")
    print(f"  Saved {len(results['referenced_images'])} referenced images to 'referenced_images.txt'")
