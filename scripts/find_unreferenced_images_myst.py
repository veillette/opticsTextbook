...
def run_myst_command(command, timeout=120):
    """Run a MyST command and capture output."""
    try:
...
def get_myst_build_references():
    ...
    output, return_code = run_myst_command(['npm', 'run', 'build'])

    # Also try a dry run build if available
    if 'myst' in output.lower():
        # Try direct myst command for more detailed info
        myst_output, _ = run_myst_command(['npx', 'myst', 'build', '--check', '--verbose'])
        output += "\n" + myst_output
...
def save_results(results, include_ai=True):
    ...
    if results['unreferenced_images']:
        with open('reports/unreferenced_images.txt', 'w', encoding='utf-8') as f:
            f.write("# Unreferenced image files\n")
            f.write(f"# Found {len(results['unreferenced_images'])} unreferenced images\n")
            f.write("# Generated by find_unreferenced_images_myst.py\n\n")
            for image in sorted(results['unreferenced_images']):
                f.write(f"{image}\n")
        print(f"  Saved {len(results['unreferenced_images'])} unreferenced images to 'reports/unreferenced_images.txt'")

    # Save unreferenced .ai files if requested
    if include_ai and results['unreferenced_ai_files']:
        with open('reports/unreferenced_ai_files.txt', 'w', encoding='utf-8') as f:
            f.write("# Unreferenced .ai source files\n")
            f.write(f"# Found {len(results['unreferenced_ai_files'])} unreferenced .ai files\n")
            f.write("# These .ai files correspond to unreferenced image files\n")
            f.write("# Generated by find_unreferenced_images_myst.py\n\n")
            for ai_file in sorted(results['unreferenced_ai_files']):
                corresponding_image = find_corresponding_image_file(ai_file)
                f.write(f"{ai_file}")
                if corresponding_image:
                    f.write(f"  # corresponds to {corresponding_image}")
                f.write("\n")
        print(f"  Saved {len(results['unreferenced_ai_files'])} unreferenced .ai files to 'reports/unreferenced_ai_files.txt'")

    # Save referenced images for verification
    with open('reports/referenced_images.txt', 'w', encoding='utf-8') as f:
        f.write("# Referenced image files (for verification)\n")
        f.write(f"# Found {len(results['referenced_images'])} referenced images\n")
        f.write("# Generated by find_unreferenced_images_myst.py\n\n")
        for image in sorted(results['referenced_images']):
            f.write(f"{image}\n")
    print(f"  Saved {len(results['referenced_images'])} referenced images to 'reports/referenced_images.txt'")
